...
async def update_offerings(
    ...
    brassica_client.update_offering(
        offering_id,
        {
            "data": {
                "attributes": {
...
async def get_offering_series(
    offering_id: str, user: Dict = Depends(granted_user)
) -> List:
...
async def create_offering_series(
    offering_series: BrassicaOfferingSeries, user: Dict = Depends(granted_user)
) -> None:
    ...
    if asset.brassica_security_id:
        ...
        for existing_security in securities:
            security_name = (
                existing_security.get("attributes", {}).get("class").replace(" ", "_")
            )
    ...
    offering_series_asset_id: str = brassica_client.create_offering_series_asset(
        security,
...
async def delete_offering_series(
    offering_id: str, offering_series_id: str, user: Dict = Depends(granted_user)
) -> List:
